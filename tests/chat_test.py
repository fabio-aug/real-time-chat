...
class ChatTests(unittest.TestCase):
    ...
    def test_image_reception(self):
        ...
        try:
            ...
            canvas_receive = WebDriverWait(driver_receive, timeout=10).until(lambda d: d.find_element(By.TAG_NAME, 'flt-glass-pane'))
            ...
            canvas_shadow = shadow_root.find_element(By.XPATH, '//canvas[1]')
            ...
            textarea_send = WebDriverWait(driver_send, timeout=10).until(lambda d: d.find_element(By.TAG_NAME, 'textarea'))
            textarea_send.send_keys('Mensagem de teste')  # Digite a mensagem na textarea
            textarea_send.send_keys(u'\ue007')
            input_send = WebDriverWait(driver_send, timeout=10).until(lambda d: d.find_element(By.TAG_NAME, 'input'))
